...
def set_time_spent(item_id):
    data = request.json
    conn = get_db()
    conn.execute("UPDATE bucket_list SET time_spent=? WHERE id=?", (data.get('time_spent') or None, item_id))
    conn.commit()
    item = conn.execute("SELECT * FROM bucket_list WHERE id=?", (item_id,)).fetchone()
    conn.close()
    return jsonify(dict(item))
...
def set_subitem_time_spent(sub_id):
    data = request.json
    conn = get_db()
    conn.execute("UPDATE bucket_subitems SET time_spent=? WHERE id=?", (data.get('time_spent') or None, sub_id))
    conn.commit()
    sub = conn.execute("SELECT * FROM bucket_subitems WHERE id=?", (sub_id,)).fetchone()
    conn.close()
    return jsonify(dict(sub))
...
def set_task_time_spent(task_id):
    data = request.json
    conn = get_db()
    conn.execute("UPDATE tasks SET time_spent=? WHERE id=?", (data.get('time_spent') or None, task_id))
    conn.commit()
    task = conn.execute("SELECT * FROM tasks WHERE id=?", (task_id,)).fetchone()
    conn.close()
    return jsonify(dict(task))
...
def export_text():
    ...
    if dropped:
        lines += ["─" * 42, "✕ やらないと決めた"]
        for item in sorted(dropped, key=dl_sort):
    ...
    def fmt_task_line(t, extra=''):
        ...
        return f"    [{'✓' if t['completed'] else '○'}] {t['title']}{extra}{cd}{ts}"

    for label, col, order, task_type in [
        ("【月次タスク】", 'target_month', 'target_month', 'monthly'),
        ("【週次タスク】", 'target_week',  'target_week',  'weekly'),
        ("【日次タスク】", 'target_date',  'target_date DESC', 'daily'),
    ]:
        lines += ["", label]
        rows = conn.execute(
            f"SELECT * FROM tasks WHERE task_type=? ORDER BY {order}, completed, created_at",
            (task_type,)
        ).fetchall()
        if rows:
            cur_val = None
            for t in rows:
                ...
                if val != cur_val:
                    cur_val = val
                    lines.append(f"\n  ▸ {val}")
                extra = " (繰越)" if task_type == 'daily' and t['rolled_over_from'] else ""
                lines.append(fmt_task_line(t, extra))
        else:
            lines.append("  （なし）")
...
def export_csv():
    ...
    for t in conn.execute(
        ...
        writer.writerow([t['id'], t['title'], type_ja.get(t['task_type'], t['task_type']),
                         t['target_date'] or '', t['target_week'] or '', t['target_month'] or '',
                         '完了' if t['completed'] else '未完了', t['rolled_over_from'] or '', t['created_at']])
